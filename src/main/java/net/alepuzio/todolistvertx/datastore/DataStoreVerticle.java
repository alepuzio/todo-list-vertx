...
public class DataStoreVerticle extends AbstractVerticle {
    ...
    public void start(Future<Void> storeFeature) {
        ...
        sqlClient.getConnection(
            ...
            if (conn.succeeded()) {
                ...
                conn.result().setOptions(new SQLOptions().setAutoGeneratedKeys(Boolean.TRUE));//set conf in current transaction

                eventBus.consumer("todo.find.all",//address
                		message -> findAll(message, conn.result()));

                eventBus.consumer("todo.find.todo", //address
                		message -> findTodo(message, conn.result()));

                eventBus.consumer("todo.create",//address 
                		message -> createTodo(message, conn.result()));

                eventBus.consumer("todo.update",//address 
                		message -> updateTodo(message, conn.result()));

                eventBus.consumer("todo.delete",//address 
                		message -> deleteTodo(message, conn.result()));
            } else {
    ...
    /**
     * 
     * @param message: channel name
     * @param connection:
     * */
    private void findAll(final Message<Object> message, final SQLConnection connection) {
        if (Objects.nonNull(message)) {
            connection.query(
            		"SELECT t.id as T_ID, t.content as T_CONTENT, t.creation_date AS T_CREATION, " +
                    ...
                    " on (u.id = t.user_id) order by t.id desc", 
                    sqlResult -> {
    ...
    private void findTodo(final Message<Object> message, final SQLConnection connection) {
        if (Objects.nonNull(message) && Objects.nonNull(message.body())) {
            ...
            connection.queryWithParams( "SELECT t.id as T_ID, t.content as T_CONTENT, t.creation_date AS T_CREATION, " +
                    "u.id as U_ID, u.username as U_USER, u.password as U_PWD, u.name as U_NAME, u.surname as U_SNAME, " +
                    "u.email as U_EMAIL, u.creation_date as U_CREATION from todo t join user u" +
                    " on (u.id = t.user_id) WHERE t.id = ? order by t.id desc", 
                    jsonParam,  
                    sqlResult -> {
                if (sqlResult.succeeded()) {
